...
bool Config::SaveConfig(const bool forceSave)
{
	...
	if (forceSave || (scheduleSave && SysTickVal > saveBooked + 60000)) {			// 60 seconds between saves
		...
		FLASH->NSCCR = flashAllErrors;						// Clear error flags in Status Register
...
void Config::FlashUnlock()
{
	...
	if ((FLASH->NSCR & FLASH_CR_LOCK) != 0)  {
		FLASH->NSKEYR = 0x45670123U;						// These magic numbers unlock the flash for programming
...
{
	FLASH->NSCR |= FLASH_CR_LOCK;						// Lock the FLASH Registers access
}
...
void Config::FlashEraseSector(uint8_t sector)
{
	...
	FLASH->NSCR |= (sector - 1) << FLASH_CR_SNB_Pos;		// Sector number selection
	FLASH->NSCR |= FLASH_CR_SER;							// Sector erase
...
{
	if (FLASH->NSSR & flashAllErrors) {					// If any error occurred abort
		FLASH->NSCCR = flashAllErrors;					// Clear error flags in Status Register
		return false;
	}

	while ((FLASH->NSSR & FLASH_SR_BSY) == FLASH_SR_BSY) {}

	if ((FLASH->NSSR & FLASH_SR_EOP) == FLASH_SR_EOP) {	// Check End of Operation flag
		FLASH->NSSR = FLASH_SR_EOP;						// Clear FLASH End of Operation pending bit
	}
...
bool Config::FlashProgram(uint32_t* dest_addr, uint32_t* src_addr, size_t size)
{
	...
	for (uint16_t b = 0; b < std::ceil(static_cast<float>(size) / 16); ++b) {
		...
		if (!FlashWaitForLastOperation()) {
			FLASH->NSCR &= ~FLASH_CR_PG;				// Clear programming flag
			return false;
		}
	}
